...
public:
	...
	FK2PostItDecorator_InlineCode(FString InName, const FSlateColor& InColor);
	
	bool Supports( const FTextRunParseResults& RunInfo, const FString& Text ) const override;

	static TSharedRef<FK2PostItDecorator_InlineCode> Create(FString InName, const FSlateColor& InColor)
	{
		return MakeShareable(new FK2PostItDecorator_InlineCode(MoveTemp(InName), InColor));
	}
